...
.. note:: **自動的にトランザクショントークンを埋め込みたいが、action属性を付与したくない場合**
    ...
    「\ :ref:`view_thymeleaf_requesturl-label`\」で解説する「ページ相対URL」を利用することで、リクエストマッピングのパスが異なる複数のコントローラで同じテンプレートHTMLを使いまわすことが可能である。
    「ページ相対URL」を使用すると、必ずページを取得したパスから派生する別のパスを指定する必要があるように見えるが、\ ``th:action``\属性の値を指定しないことで、出力される\ ``action``\属性の値が空になり、ページを取得したのと同じパスに対してリクエストを送信することが可能となる。
